...
describe("parseLockUntilInput", () => {
  // Anchor "now" so the assertions don't shift with wall-clock.
  // 2026-01-01T00:00:00Z in local-time-readable form depends on the test
  // machine's tz, so we keep math in unix ms and feed values relative to it.
  ...
  it("returns null for a target in the past", () => {
    // Local-time "datetime-local" string one day BEFORE now (in UTC ms).
    // The exact local-time string varies by tz, but the resulting
    // duration is negative, which the parser must reject.
  ...
  it("returns null for a target less than the 1-hour minimum", () => {
    // 30 minutes out — under MIN_LOCK_SECONDS
  ...
  it("accepts a target at the minimum (1 hour out)", () => {
    ...
    // Rounding via the toDatetimeLocalValue → parse cycle drops sub-minute
    // precision, so just assert the floor matches expectations.
  ...
  it("accepts a target a week out and returns the right duration", () => {
    ...
    const result = parseLockUntilInput(value, NOW_MS);
    expect(result).not.toBeNull();
    // Allow a minute of rounding slack from the "YYYY-MM-DDTHH:MM" precision.
...
describe("toDatetimeLocalValue", () => {
  ...
  it("round-trips through new Date() in local time", () => {
    const d = new Date();
    d.setSeconds(0, 0); // drop sub-minute precision the input doesn't carry
    const out = toDatetimeLocalValue(d);
